...
class DashboardViewTests(TestCase, PortfolioTestMixin):
    ...
    def test_dashboard_calculated_values(self) -> None:
        ...
        TargetAllocation.objects.create(user=self.user, account_type=acc_tax.account_type, asset_class=ac_us, target_pct=50)
        ...
        ac_intl, _ = AssetClass.objects.get_or_create(name='Intl Stocks', defaults={'category': cat_eq})
        sec_intl, _ = Security.objects.get_or_create(ticker='VXUS', defaults={'name': 'VXUS', 'asset_class': ac_intl})
        Holding.objects.create(account=acc_tax, security=sec_intl, shares=10, current_price=50)
        # Add target for Intl: 40%
        TargetAllocation.objects.create(user=self.user, account_type=acc_tax.account_type, asset_class=ac_intl, target_pct=40)
